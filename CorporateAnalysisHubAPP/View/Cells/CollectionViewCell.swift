...
class IndexCollectionViewCell: UICollectionViewCell {
    ...
    func createLayout() {
        ...
        indexNameLabel = UILabel(frame: CGRect(x: 8, y: 8, width: bounds.size.width - 16, height: (self.bounds.size.height - 24) * 4.5 / 10))

        let w_h = self.bounds.size.height - indexNameLabel.frame.size.height - 40
        stateImageView = UIImageView(frame: CGRect(x: bounds.size.width - w_h - 8, y: indexNameLabel.frame.maxY + 16, width: w_h, height: w_h))

        indexValueLabel = UILabel(frame: CGRect(x: 8, y: indexNameLabel.frame.maxY + 8, width: bounds.size.width - 24 - w_h, height: self.bounds.maxY - indexNameLabel.frame.maxY - 16))

        separatorView = UIView(frame: CGRect(x: 8, y: indexNameLabel.frame.maxY + 4, width: bounds.size.width - 8, height: 0.2))
        ...
        stateImageView.image = UIImage(systemName: "exclamationmark.circle")
        // exclamationmark.circle // multiply.circle // checkmark.circle
...
class ArticleCell: UICollectionViewCell {
    ...
    func createLayout() {
        ...
        indexValueLabel = UILabel(frame: CGRect(x: 8, y: indexNameLabel.frame.maxY + 8, width: self.bounds.size.width - 24 - w_h, height: bounds.maxY - indexNameLabel.frame.maxY - 16))

        separatorView = UIView(frame: CGRect(x: 8, y: indexNameLabel.frame.maxY + 4, width: frame.size.width - 8, height: 0.8))

        indexNameLabel.font = UIFont.systemFont(ofSize: indexNameLabel.bounds.size.height * 0.6, weight: .medium)
        indexValueLabel.font = UIFont.systemFont(ofSize: indexValueLabel.bounds.size.height * 0.6, weight: .medium)
        indexValueLabel.textAlignment = .right
        stateImageView.image = UIImage(systemName: "multiply.circle")
        // exclamationmark.circle // multiply.circle // checkmark.circle
