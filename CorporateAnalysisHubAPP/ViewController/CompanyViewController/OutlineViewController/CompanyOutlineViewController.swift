...
extension CompanyOutlineViewController: UICollectionViewDelegate {
    ...
    func barChartsConfig(barChartView: BarChartView, rawData: [Double], years: [String]) {
    ...
    func applyInitialSnapshots() {
        ...
        let important_type: [CompanyFinIndexData.Tag] = [.ROE, .ROIC, .ROA, .equityRatio]
        let safety_type: [CompanyFinIndexData.Tag] = [.currentRatio, .shortTermLiquidity, .fixedAssetsToNetWorth, .fixedAssetToFixedLiabilityRatio, .debtEquityRatio, .netDebtEquityRation, .dependedDebtRatio]
        let profitability_type: [CompanyFinIndexData.Tag] = [.grossProfitMargin, .operatingIncomeMargin, .ordinaryIncomeMargin, .netProfitMargin, .netProfitAttributeOfOwnerMargin]
        let efficiency_type: [CompanyFinIndexData.Tag] = [.totalAssetsTurnover, .receivablesTurnover, .inventoryTurnover, .payableTurnover, .tangibleFixedAssetTurnover, .CCC]
        let cfAnalysis_type: [CompanyFinIndexData.Tag] = [.netSalesOperatingCFRatio, .equityOperatingCFRatio, .operatingCFCurrentLiabilitiesRatio, .operatingCFDebtRatio, .fixedInvestmentOperatingCFRatio]
        var important_items: [IndexDataItem] = []
        var safety_items: [IndexDataItem] = []
        var profitability_items: [IndexDataItem] = []
        var efficiency_items: [IndexDataItem] = []
        var cfAnalysis_items: [IndexDataItem] = []
...
extension CompanyOutlineViewController {
    ...
    private struct TransitionItem: Hashable {
        ...
        func data() -> (BarChartDataSet, [Double]) {
            var rawData: [Double] = []
            switch type {
            case .Revenue:
                for i in 0 ..< keys.count {
                    let data = company.finDataDict[keys[i]]!.pl!.getNetSales_etcValue()
                    let data2 = Double(data.0 / 1000000)
